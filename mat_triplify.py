...
import csv
import rdflib
from rdflib import Literal, BNode, Namespace, URIRef, RDF, XSD
import uuid
...
def main():
    ...
    with open(csvfile, 'rb') as f:
        ...
        for row in reader:
            ...
            ds.add((thicknessid, QUDT.numericValue, Literal(row[4],
                                                            datatype=XSD.float)))
            ...
            ds.add((embodiedenergy, QUDT.numericValue, Literal(row[5],
                                                               datatype=XSD.float)))
            if (row[6] == '1'):
            ...
            ds.add((URIRef(componentid),
                    COMPONENT.hasMaterialDensity, materialdensityArch))
